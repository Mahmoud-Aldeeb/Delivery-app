            ...
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
              ...
              {partners.length === 0 ? (
                ...
                <div className="space-y-2 mb-5 max-h-60 overflow-y-auto">
                  {partners.map((p) => (
                    <label htmlFor={`partner-${p.id}`} key={p.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${selectedPartner === p.id ? "border-app-green bg-app-green/5" : "border-app-border hover:bg-app-cream"}`}>
                      <input type="radio" id={`partner-${p.id}`} name="partner" value={p.id} checked={selectedPartner === p.id} onChange={() => setSelectedPartner(p.id)} className="text-app-green" />
